...
export const initCommand = new Command('init')
  .description('Initialize @kodda/ui in your project')
  .action(async () => {
    const cwd = process.cwd();

    // Check if already initialized
    if (await fs.pathExists(path.join(cwd, 'kodda.json'))) {
      console.log(chalk.yellow('⚠ kodda.json already exists. Project is already initialized.'));
      return;
    }
    ...
    if (await fs.pathExists(pkgJsonPath)) {
      ...
      if (missing.length > 0) {
        ...
        console.log(chalk.dim('  @kodda/ui components require React and Tailwind CSS.'));
      }
    }
    ...
    const response = await prompts([
      ...
      },
      {
        type: 'text',
        name: 'alias',
        message: 'Import alias for kodda components?',
        initial: '@/components/kodda',
      },
    ]);
    ...
    config.componentDir = `${response.componentDir}/kodda`;
    ...
    try {
      ...
      console.log();
      console.log(chalk.green('✔'), 'Created', chalk.bold('kodda.json'));
      for (const dir of dirs) {
        console.log(chalk.green('✔'), 'Created', chalk.dim(`${config.componentDir}/${dir}/`));
      }
      console.log();
      console.log('Next: run', chalk.cyan('npx @kodda/ui add <component>'), 'to add components.');
    } catch (error) {
