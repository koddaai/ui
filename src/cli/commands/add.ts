...
export const addCommand = new Command('add')
  .description('Add kodda components to your project')
  ...
  .action(async (componentNames: string[], options: { local?: boolean; yes?: boolean }) => {
    ...
    if (resolved.kodda.length === 0 && resolved.shadcn.length === 0 && resolved.npm.length === 0) {
    ...
    if (resolved.kodda.length > 0) {
      console.log(
        chalk.dim('  kodda:'),
        resolved.kodda.map((k) => k.slug).join(', '),
      );
    }
    ...
    for (const { slug, component } of resolved.kodda) {
      ...
      try {
        ...
        componentSpinner.succeed(
          `kodda: ${slug} → ${config.componentDir}/composites/${slug}/`,
        );
      } catch (error) {
    ...
    console.log(
      chalk.green('✔'),
      chalk.bold(`${resolved.kodda.length} component(s) added.`),
    );
  });
