...
import { saveConfig, loadConfig, KoddaConfig } from '../../../src/cli/utils/config.js';
...
const mockRegistry: Registry = {
  ...
  components: {
    'status-badge': {
      ...
      dependencies: { shadcn: ['badge'], kodda: [], npm: [] },
    },
    'filter-bar': {
      name: 'FilterBar',
      ...
      dependencies: { shadcn: ['input', 'select', 'button'], kodda: [], npm: [] },
    },
    'data-table': {
      name: 'DataTable',
...
describe('add command logic', () => {
  ...
  let config: KoddaConfig;

  beforeEach(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kodda-add-test-'));
    sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kodda-source-'));
  ...
  it('adds a simple component without kodda deps', async () => {
    const resolved = resolveDependencies(['status-badge'], mockRegistry, config.installed);
    expect(resolved.kodda).toHaveLength(1);
    expect(resolved.kodda[0].slug).toBe('status-badge');

    // Install the component
    for (const { slug, component } of resolved.kodda) {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, 'src/components/kodda/composites/status-badge/status-badge.tsx'),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, 'src/components/kodda/composites/status-badge/index.ts'),
      ),
    ).toBe(true);
  });
  ...
  it('adds a component with transitive deps', async () => {
    ...
    const slugs = resolved.kodda.map((k) => k.slug);
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, 'src/components/kodda/composites/data-table/data-table.tsx'),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, 'src/components/kodda/composites/filter-bar/filter-bar.tsx'),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it('updates kodda.json after installation', async () => {
    const resolved = resolveDependencies(['status-badge'], mockRegistry, config.installed);
  ...
  it('does not overwrite existing component files', async () => {
    ...
    const existingPath = path.join(
      tmpDir,
      'src/components/kodda/composites/status-badge/status-badge.tsx',
    );
