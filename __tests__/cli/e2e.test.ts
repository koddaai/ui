...
describe('e2e: init + add in clean project', () => {
  ...
  beforeEach(async () => {
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kodda-e2e-'));
  ...
  it('init creates config and directory structure', async () => {
    ...
    config.componentDir = 'src/components/kodda';
    config.alias = '@/components/kodda';
    ...
    // Verify
    expect(fs.existsSync(path.join(projectDir, 'kodda.json'))).toBe(true);
    for (const dir of dirs) {
      expect(fs.existsSync(path.join(projectDir, config.componentDir, dir))).toBe(true);
    }

    const loaded = await loadConfig(projectDir);
    expect(loaded.componentDir).toBe('src/components/kodda');
  ...
  it('add resolves deps and installs status-badge from real registry', async () => {
    ...
    expect(resolved.kodda.length).toBeGreaterThanOrEqual(1);
    expect(resolved.shadcn).toContain('badge');

    // Install kodda components
    for (const { slug, component } of resolved.kodda) {
      await installComponent(component, slug, config, { sourceDir: repoRoot }, projectDir);
