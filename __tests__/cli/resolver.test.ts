...
const mockRegistry: Registry = {
  ...
  components: {
    ...
    'circular-a': {
      ...
      version: '0.1.0',
      files: [],
      dependencies: { shadcn: [], kodda: ['circular-b'], npm: [] },
    },
    'circular-b': {
      name: 'CircularB',
      ...
      version: '0.1.0',
      files: [],
      dependencies: { shadcn: [], kodda: ['circular-a'], npm: [] },
    },
  },
};

describe('resolveDependencies', () => {
  it('resolves a simple component without kodda deps', () => {
    const result = resolveDependencies(['status-badge'], mockRegistry);
    expect(result.kodda).toHaveLength(1);
    expect(result.kodda[0].slug).toBe('status-badge');
    expect(result.shadcn).toEqual(['badge']);
    expect(result.npm).toEqual([]);
  });

  it('resolves transitive kodda dependencies', () => {
    const result = resolveDependencies(['data-table'], mockRegistry);
    const slugs = result.kodda.map((k) => k.slug);
