...
description: Template de página de login do dashboard com botão OAuth e magic link. Tema escuro com branding kodda.
---
...
```bash
npx @koddaai/ui add dashboard-login-page
```

## Importação

```tsx
import { DashboardLoginPage } from "@/components/kodda/templates/dashboard-login-page"
```

## Fluxo de Login

O template implementa o mesmo fluxo do dashboard kodda Billing:

1. **Botão OAuth** — Login via provedor externo (ex: kodda, Google).
...
<DashboardLoginPage
  ...
  onLogin={() => loginWithkodda("/dashboard")}
...
<DashboardLoginPage
  labels={{
    ...
    loginWithProvider: "Sign in with kodda",
